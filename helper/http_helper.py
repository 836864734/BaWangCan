...
from util.random_sleep_util import RandomSleepUtil


class HttpHelper:
    logger = Logger.getLogger("HttpHelper")
    # 查询指定城市的全部符合类别的霸王餐
    def list(self, cityId="1"):
        activitys = []
    ...
    # 批量报名
    def regist(self, list: [], user: User) -> []:
        feedbacks = []
        successCount = 0
        registedCount = 0
        errCount = 0
        for activity in list:
            ...
            if("不要重复报名") in msg["msg"]:
                ...
                feedbacks.append(FeedBack(user.name, activity.title, "报名异常", Configurator.desc_url + str(activity.id), msg["msg"]["html"]))
                errCount += 1

            self.logger.info("用户%s成功报名%s条活动", user.name, successCount)
            self.logger.info("用户%s重复报名%s条活动", user.name, registedCount)
            self.logger.info("用户%s报名失败%s条活动", user.name, errCount)
            # 随机休眠2-10秒
            RandomSleepUtil.sleep(2, 10)
